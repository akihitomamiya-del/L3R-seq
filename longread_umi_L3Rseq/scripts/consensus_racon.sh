...
#    Raw read centroid found with usearch/vsearch and used as seed for
...
#    - Replaced hardcoded '-fastaout' in sortbysize with $USEARCH_SORTOUT_FLAG
#      for vsearch compatibility (vsearch uses --output instead)
...
seed_racon () {
  ...
  # Find seed read: pick median-length read (deterministic, engine-independent).
  # Median is preferred over longest because the longest read is more likely to
  # contain insertions from sequencing errors. Ties broken by lexicographic
  # sequence content for full reproducibility across runs.
  $GAWK '
    NR%4==1 { name = $0; next }
    NR%4==2 { len[name] = length($0); seq[name] = $0; names[++n] = name; next }
    NR%4==3 { next }
    NR%4==0 { next }
    END {
      # Sort by length descending, then sequence lexicographic
      for (i = 1; i <= n; i++)
        for (j = i+1; j <= n; j++)
          if (len[names[i]] < len[names[j]] ||
              (len[names[i]] == len[names[j]] && seq[names[i]] > seq[names[j]])) {
