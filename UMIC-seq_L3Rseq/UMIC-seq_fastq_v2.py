...
#14/09/2022 UMICseq_fastq_v2
##export umi[cluster No.]bins.fastq files after clustering (instead of cluster[Cluster No.].fasta)
...
def cluster_sequences(umis, reads, aln_thresh, size_thresh, max_clusters=0, clussize_thresh=0, clussize_window=20):
    ...
    for i in range(clus_N):
        if cluster_sizes[i] >= size_thresh:
            ...
            fname = output_folder + "/umi" + str(count) + "bins.fastq" #FASTA OR FASTQ? -> FASTQ in UMICseq_fastq_v2: umi[cluster No.]bins.fastq to meet longread_umi consensus_racon requirements.
